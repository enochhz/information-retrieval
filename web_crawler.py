...
class WebCralwer:
    ...
    def store_info_in_csv(self):
        ...
        id_Nv_df = pd.DataFrame(columns=['id', 'Nv'])
        ...
        for url, link_to_urls in self.visited_page_map.items():
            ...
            url_id = self.visited_pages_id[url] #visited_pages_id a map of url and id
            ...
            pandas_df.to_csv('out_links/' + str(url_id) + '.csv')

            id_Nv_df = id_Nv_df.append({
                'id' : url_id,
                'Nv': len(link_to_urls_ids)
            }, ignore_index=True);

        url_id_df.to_csv('url_id.csv')
        id_Nv_df.to_csv('url_Nv.csv')

    def reverse_counting(self):
        for path in pathlib.Path("out_links").iterdir():
            if path.is_file():
                current_file = open(path, "r")
                print(current_file.read())
                current_file.close()
        # file = open('out_links/' + str(url_id) + '.csv')
        # reverse_counted_df = pd.DataFrame(columns=['link_to_id', 'link_from_ids'])
        
        # for url, link_to_urls in self.visited_page_map.items():
        #     for link_to in link_to_urls:
